export const projectData = [
   {
      name: 'Shelter',
      image: 'assets/img/shelter.png',
      link: 'https://ksenchik.github.io/Shelter/shelter/pages/main/',
      description: 'Adaptive layout. Used: HTML, CSS, JS, jQuery',
      code: 'https://github.com/Ksenchik/Shelter/tree/main/shelter'
   },
   {
      name: 'Online-zoo',
      image: 'assets/img/online-zoo.png',
      link: 'https://ksenchik.github.io/PetStory-online/online-zoo/pages/main/',
      description: 'Adaptive layout. Used: HTML, SCSS, JS, jQuery. Done: validation HTML form, JS functional',
      code: 'https://github.com/Ksenchik/PetStory-online/tree/main/online-zoo'
   },
   {
      name: 'Songbird',
      image: 'assets/img/songbird.png',
      link: 'https://rolling-scopes-school.github.io/ksenchik-JSFE2022Q3/songbird/dist/',
      description: 'Adaptive layout. Used: HTML, SCSS, JS. Done: SPA, routing, game logic',
      code: 'https://github.com/Ksenchik/Songbird/tree/main/songbird'
   },
   {
      name: 'Online-store',
      image: 'assets/img/online-store.png',
      link: 'https://rolling-scopes-school.github.io/ksenchik-JSFE2022Q3/songbird/dist/',
      description: 'Adaptive layout. Used: HTML, SCSS, JS',
      code: 'https://github.com/Ksenchik/Online-store/tree/develop/public/template'
   }
];